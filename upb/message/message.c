...
void upb_Message_Freeze(upb_Message* msg, const upb_MiniTable* m) {
  ...
  UPB_PRIVATE(_upb_Message_Freeze)(msg);
  ...
  for (size_t i = 0; i < field_count; i++) {
    ...
    if (upb_MiniTableField_IsArray(f)) {
      upb_Array_Freeze((upb_Array*)val.array_val, m, f);
      continue;
    }
    if (upb_MiniTableField_IsMap(f)) {
      upb_Map_Freeze((upb_Map*)val.map_val, m, f);
      continue;
    }
    if (upb_MiniTableField_IsSubMessage(f)) {
      const upb_MiniTable* m2 = upb_MiniTable_GetSubMessageTable(m, f);
      upb_Message_Freeze((upb_Message*)val.msg_val, m2);
      continue;
    }
  }
