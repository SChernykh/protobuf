...
#define UPB_OPAQUE(x) x##_visible_within_this_header_only
...
UPB_INLINE void UPB_PRIVATE(_upb_Message_Freeze)(struct upb_Message* msg) {
  msg->UPB_OPAQUE(internal) |= 1;
}

UPB_INLINE bool UPB_PRIVATE(_upb_Message_IsFrozen)(
    const struct upb_Message* msg) {
  return (msg->UPB_OPAQUE(internal) & 1) != 0;
}

UPB_INLINE struct upb_Message_Internal* UPB_PRIVATE(_upb_Message_GetInternal)(
    const struct upb_Message* msg) {
  const uintptr_t tmp = msg->UPB_OPAQUE(internal) & ~1;
